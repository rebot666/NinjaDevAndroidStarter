...
public class APIConnection {
    ...
    public static JSONObject getMangaInfo(String mangaId) throws IOException, JSONException {
        Request request = new Request.Builder()
                .url(getMangaChaptersURL + mangaId)
                .build();
        Response response = client.newCall(request).execute();
        return new JSONObject(response.body().string());
    }

    public static JSONObject getListOfMangaByIndex(String index) throws IOException, JSONException {
        Request request = new Request.Builder()
                .url(getListOfMangaByIndexURL + "?p=" + index)
                .build();
        Response response = client.newCall(request).execute();
        return new JSONObject(response.body().string());
    }

    public static JSONObject getListOfManga() throws IOException, JSONException {
        Request request = new Request.Builder()
                .url(getListOfMangaByIndexURL)
                .build();
        Response response = client.newCall(request).execute();
        return new JSONObject(response.body().string());
    }
